...
## 📅 Sprint 1 Deliverables & Release Notes (v1.0.0)

This release represents the complete deliverables for **Sprint 1** (Prototype phase):

### 🌟 Implemented in Sprint 1
* **Multi-Modal Requirements Parsing Engine**: Ingestion pipeline for `.pdf`, `.docx`, `.txt`, and `.png`/`.jpg` wireframes.
* **Intelligent Synthesis**: Integration with Google Gemini 1.5 Pro to synthesize unstructured notes into standard functional/non-functional requirements tables.
* **Pairwise Conflict Detector**: Scans requirements to automatically surface contradictory clauses (e.g., SSO vs username/password constraints).
* **Source Traceability & Confidence Rating**: Tracks requirement sources and calculates confidence levels (HIGH/MEDIUM/LOW).
* **BigQuery DB Simulation**: SQLite-backed project archiver supporting save, load, version history, and deletion.
* **Styled Exporters**: One-click generation of ReportLab-powered PDF reports (complete with cover page and page numbers) and editable Word (`.docx`) files.

### 🔄 Latest Changes & Refinements
* **Streamlit API Fix**: Resolved the `TypeError: ButtonMixin.button() got an unexpected keyword argument 'kind'` by migrating button components to use Streamlit's official `type="secondary"` parameter.
* **Dynamic Connection Mode**: Enabled seamless toggling between simulated offline Demo Mode (`DEMO_MODE=True` in `.env`) and live Gemini API execution (`DEMO_MODE=False`) with immediate indicator status in the sidebar.
* **Mermaid Render Correction**: Fixed sub-graph syntax errors inside the repository document rendering tool to make sure diagrams load properly on GitHub.
* **Environment Security**: Configured `.gitignore` to protect sensitive local databases (`*.db`) and API configuration keys (`.env`) from public leaks.
