...
import database.bigquery_handler as bigquery_handler
...
from core.gcs_storage import is_gcs_enabled, upload_file_to_gcs
...
async def get_current_user(request: Request) -> dict:
    ...
    if is_firebase_enabled():
        try:
            ...
            logger.warning(f"Firebase token verification failed; trying local demo JWT fallback: {e}")

    # Fallback Local JWT Decryption. This keeps the demo login working even when
    # Firebase Admin/Firestore are enabled for cloud storage.
    try:
...
@app.get("/api/system/bigquery-test")
def bigquery_test():
    """
    Initializes BigQuery and performs a tiny write/read check.
    """
    if not config.ENABLE_BIGQUERY:
        raise HTTPException(status_code=400, detail="ENABLE_BIGQUERY is false")

    if not bigquery_handler.init_bigquery():
        raise HTTPException(status_code=502, detail=bigquery_handler.get_bigquery_status())

    try:
        test_payload = {
            "project_name": "BigQuery Connectivity Test",
            "domain": "System",
            "executive_summary": "Connectivity test row generated by BRD Forge.",
            "functional_requirements": [
                {
                    "id": "FR-BQ-001",
                    "title": "Persist BRD row",
                    "description": "System can write and read a BRD record in BigQuery.",
                    "source": "system",
                    "confidence": "HIGH",
                    "conflict": None,
                    "priority": "MUST HAVE",
            ...
            "risks": [],
            "assumptions": [],
            "acceptance_criteria": ["BigQuery write/read test succeeds."],
        }
        project_id = bigquery_handler.save_brd_bigquery("BigQuery Connectivity Test", test_payload)
        loaded = bigquery_handler.get_brd_by_id_bigquery(project_id)
        return {
            "status": "success",
            "project_id": project_id,
            "loaded": bool(loaded),
            "bigquery": bigquery_handler.get_bigquery_status(),
        }
    except Exception as e:
    ...
    try:
        ...
        if files:
            ...
            for file in upload_files:
                ...
                cloud_uri = upload_file_to_gcs(local_path, remote_prefix="brd-inputs")
                if cloud_uri:
                    uploaded_artifacts.append({
                        "filename": filename,
                        "local_path": local_path,
                        "cloud_uri": cloud_uri,
                        "storage": "gcs" if str(cloud_uri).startswith("gs://") else "local"
                    })
        ...
        brd_payload["uploaded_artifacts"] = uploaded_artifacts
...
def delete_project(project_id: str, current_user: dict = Depends(get_current_user)):
    try:
        result = db_manager.delete_brd(project_id)
        return {
            "status": "success",
            "detail": f"Project {project_id} deleted if present.",
            **(result or {})
        }
    except HTTPException as he:
        raise he
